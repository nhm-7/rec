"""Define all code related to datasets."""
...
def collate_fn(batch):
    """Collate a batch of samples into batched tensors for the dataloader."""
    ...
    if batch[0]["tok"] is not None:
        tok = {
            ...
            "attention_mask": torch.cat(
                [s["tok"]["attention_mask"] for s in batch], dim=0
            ),
        }
...
class RECDataset(torch.utils.data.Dataset):
    """Represent a generic dataset for referring expression comprehension tasks."""

    def __init__(
        self, transform=None, tokenizer=None, max_length=32, with_mask_bbox=False
    ):
        """Initialize RECDataset with transforms, tokenizer, and options."""
        super().__init__()
    ...
    def tokenize(self, inp, max_length):
        """Tokenize an input string using the provided tokenizer."""
    ...
    def print_stats(self):
        """Print statistics about the dataset expressions and sample count."""
        print(f"{len(self.samples)} samples")
        lens = [len(expr.split()) for _, expr, _ in self.samples]
        print(
            "expression lengths stats: "
    ...
    def __getitem__(self, idx):
        """Get a sample by index, loading and processing the image and data."""
...
class RegionDescriptionsVisualGnome(RECDataset):
    """Represent the Visual Genome region descriptions dataset."""
    ...
    def __init__(
        ...
        """Initialize RegionDescriptionsVisualGnome with data root and options."""
        super().__init__(
            transform=transform,
            tokenizer=tokenizer,
            max_length=max_length,
            with_mask_bbox=with_mask_bbox,
        )
        # if available, read COCO IDs from the val, testA and testB splits from
        # the RefCOCO dataset
        try:
            with open("./refcoco_valtest_ids.txt") as fh:
        ...
        with open(os.path.join(data_root, "image_data.json")) as f:
            image_data = {
        ...
        with open(os.path.join(data_root, "region_descriptions.json")) as f:
            for record in progressbar(
                ijson.items(f, "item.regions.item"), desc="loading data"
            ):
...
class ReferDataset(RECDataset):
    """Represent the ReferIt-style dataset for referring expression comprehension."""
    ...
    def __init__(
        ...
        """Initialize ReferDataset with data root and options."""
        super().__init__(
            transform=transform,
            tokenizer=tokenizer,
            max_length=max_length,
        ...
        try:
            ...
            raise RuntimeError(
                "create a symlink to valid refer compilation "
...
class RefCLEF(ReferDataset):
    """Represent the RefCLEF dataset variant."""

    def __init__(self, *args, **kwargs):
        """Initialize RefCLEF with split and options."""
        assert args[0] in ("train", "val", "test")
        super().__init__("refer/data", "refclef", "berkeley", *args, **kwargs)


class RefCOCO(ReferDataset):
    """Represent the RefCOCO dataset variant."""

    def __init__(self, *args, **kwargs):
        """Initialize RefCOCO with split and options."""
...
class RefCOCOg(ReferDataset):
    """Represent the RefCOCOg dataset variant."""

    def __init__(self, *args, **kwargs):
        """Initialize RefCOCOg with split and options."""
        assert args[0] in ("train", "val", "test")
