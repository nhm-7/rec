"""
Main entry point and utilities for prediction pipeline.

This module provides functions for IOU calculation, batch processing, testing, argument parsing, and dataset splitting.
...
from rec.datasets import RefCLEF, RefCOCO, RefCOCOg, RefCOCOp, collate_fn
...
from rec.utils import cprint, get_rec_counts, get_tokenizer, progressbar
...
def process_batch(batch, model, device):
    """Process a batch of data through the model and return predictions."""
...
def test(model, loader, rec, iou_threshold=0.5):
    """Evaluate the model on the test dataset and return results."""
...
def get_args():
    """Parse and return command-line arguments for the prediction script."""
    ...
    parser.add_argument("checkpoint", help="trained model", type=str)
    parser.add_argument(
        "--params",
    ...
    parser.add_argument(
        "--max-length", help="if not set, read it from the checkpoint file", type=int
    )
    parser.add_argument(
        "--input-size", help="if not set, read it from the checkpoint file", type=int
    )
    parser.add_argument(
        "--iou-threshold", help="IOU threshold", type=float, default=0.5
    )
    parser.add_argument("--batch-size", help="batch size", type=int, default=16)
    parser.add_argument("--gpus", help="GPU id", type=str)
    parser.add_argument("--num-workers", help="dataloader num workers", type=int)
    parser.add_argument(
        "--get-sample",
    ...
    parser.add_argument(
        "--use-visual-emb",
        help=(
            "if set, predict using or not the visual embeddings. "
            "Possible values are: training, yes, no. Default: training"
        ),
        type=str,
        default="training",
    )
    parser.add_argument(
        "--use-visual-pos-emb",
        help=(
            "if set, predict using or not the visual position embeddings. "
            "Possible values are: training, yes, no. Default: training"
        ),
        type=str,
        default="training",
...
def get_split_ds_class(dataset, split_arg):
    """Return the dataset class for a given split name."""
    dataset_cls_splits = {
    ...
    splits_d = {"valid": ["val"], "test": cls_split[1], "all": ["val"] + cls_split[1]}
...
def run():
    """Run the prediction pipeline with the provided arguments."""
    args = get_args()
    ...
    if args.params:
        ...
        with open(args.params) as lines:
            for line in lines:
                params = ast.literal_eval(line)
            dataset, max_length, input_size = (
                params["dataset"],
                params["max_length"],
                params["input_size"],
            )
            backbone, num_heads, num_layers = (
                params["backbone"],
                params["num_heads"],
                params["num_layers"],
            )
            num_conv, mu, mask_pooling = (
                params["num_conv"],
                params["mu"],
                params["mask_pooling"],
            )
    ...
    with open(
        args.params.replace("params.log", f"params{date_predict}.log"), "w"
    ) as fh:
    ...
    if visual_pos_emb_args:
        vis_pos_emb = get_embedding_instance(
            visual_pos_emb_args["name"], visual_pos_emb_args["args"]
        )
        ...
        if dump_results:
            df_results.to_parquet(
                args.checkpoint.replace(
                    "best.ckpt", f"predictions_{split}{date_predict}.parquet"
                ),
                index=False,
            )
            df_counts.to_csv(
                args.checkpoint.replace(
                    "best.ckpt", f"counts_{split}{date_predict}.csv"
                ),
                index=False,
            )
        print(df_counts)
