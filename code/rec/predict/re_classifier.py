"""Define all code related to refering expression classification."""
...
class REClassifier:
    """Classify referring expressions as spatial, ordinal, or relational."""

    def __init__(self, backend="stanza", device=None):
        """Initialize REClassifier with NLP backend and device."""
    ...
    def is_spatial(self, doc):
        """Check if the expression contains spatial prepositions or keywords."""
    ...
    def is_ordinal(self, doc):
        """Check if the expression contains ordinal entities."""
        if isinstance(doc, str):
            doc = self.nlp(doc)
    ...
    def is_relational(self, doc):
        """Check if the expression is relational (adposition between nouns)."""
        if isinstance(doc, str):
            doc = self.nlp(doc)
        valid_noun = [
            bool(w.pos_ in ("NOUN",) and w.text not in self._EXCLUDE) for w in doc
        ]
        relationals = [
            w.text
            for w in doc
    ...
    def classify(self, expr):
        """Classify an expression as spatial, ordinal, and/or relational."""
        doc = self.nlp(expr)
    ...
    def count_spatial_terms_separated(sent, nlp):
        """Count spatial prepositions and keywords in a sentence."""
        if isinstance(sent, str):
            doc = nlp(sent)
        else:
            doc = sent
        ngrams = [
            w.text.lower() for w in extract.ngrams(doc, (1, 2, 3), filter_stops=False)
        ]
        counts = {"prepositions": Counter(), "keywords": Counter()}
        ...
        counts["prepositions"] = {
            k: v for k, v in counts["prepositions"].items() if v > 0
        }
...
def main():
    """Run REClassifier on the test split and print classification stats."""
    ...
    print(
        f"intrinsic: {len(intrinsic)}, {np.mean(intrinsic):.2f} ({np.std(intrinsic):.2f})"
    )
    ...
    print(
        f"relational: {len(relational)}, {np.mean(relational):.2f} ({np.std(relational):.2f})"
    )
